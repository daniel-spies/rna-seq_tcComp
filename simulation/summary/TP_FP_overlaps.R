#!/bin/Rscript
## set paths and load identifier filesdir <- "~/Documents/phd/data/simulation/study"
...
for (exp in c("TP","FP"))
{
    ...
    colnames(outData) <- length(overlapData):1
    rownames(outData) <- names(overlapData)
    ...
    outFile <- file.path(dir,paste0("stats//methods_overlap_",exp,".txt"))
