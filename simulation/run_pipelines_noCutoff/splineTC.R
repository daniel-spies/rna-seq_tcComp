...
for (idx in 1:(length(args)-1))
{
    ...
    diffExprs <- splineDiffExprs(eSetObject = data, df = TP-1, reference = type[1],intercept = TRUE)
    outData <- diffExprs[,c("P.Value","adj.P.Val")]
    outData <- outData[order(outData$P.Value),]
    outFile <- sub("(contr|treat).txt","splineTC.txt",basename(args[idx]))
    write.table(outData,paste(args[length(args)],outFile,sep="/"),row.names=T,col.names=F,sep="\t",quote=F)
}