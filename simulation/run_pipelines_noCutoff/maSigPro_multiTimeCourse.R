...
for (idx in 1:(length(args)-1))
{
    ...
    NBp <- p.vector(data,design=make.design.matrix(mat,degree=deg),counts=TRUE,Q=1)
    NBt <- T.fit(NBp,step.method=meth)
    outData <- NBt$sol[,c(1,2)]
    outData <- outData[order(outData[,2],decreasing=T),]
    write.table(outData,paste(args[length(args)],outFile,sep="/"),row.names=T,col.names=F,quote=F)
}