...
do
    for exp in $(ls find /cluster/home04/biol/dspies/data/simulation_study -mindepth 1 -type d)
    do
        queue=$(basename $exp)
        ...
        do
            ...
            sleep 1 ## in order that jobs are already submited, otherwise the dependency might throw an error 
        done
    done
done