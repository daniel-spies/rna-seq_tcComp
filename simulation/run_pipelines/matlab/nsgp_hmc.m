function result = nsgp_hmc(file,nCPU,walltime)
    ...
    parfor i = 1:length(data.contr_i)
        ...
        [gpmodel,samples,mll,mse,nmse,nlpd] = nsgp(labels,contr, 'l', method);
        MLL(2)=max(mll);
        [gpmodel,samples,mll,mse,nmse,nlpd] = nsgp(labels,treat, 'l', method);
        MLL(3)=max(mll);
        [gpmodel,samples,mll,mse,nmse,nlpd] = nsgp([labels;labels],[contr;treat], 'l', method);
        MLL(1)=max(mll);
    ...
    outFile=strrep(name,'_formated','_nsgp_hmc_result.mat');
